...
class Background extends AsyncTask<String,Void,String> {
    ...
    protected String doInBackground(String... params) {
        ...
        String reqcall_url="http://imranshaik.000webhostapp.com/getdata.php";
        String track_url="http://imranshaik.000webhostapp.com/track.php";


        String method = params[0];
    String username,useremail,userpass,userphone,data,usercat,call;
        ...
        if(method.equals("userregister")) {
            ...
            try {
                ...
                return connection(reqcall_url,data);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                progressDialog.dismiss();
            }
        }
        else  if (method.equals("attempt")) {
            String id=params[1];
             phone=params[2];
            ...
            try {
                ...
                return connection(track_url,data);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
                progressDialog.dismiss();
            }
               ...
               case "registered"      :
                ...
                case "otp page"             :designation = json.getString("designation");
                                                    ...
                                                    ctx.startActivity(intent);
                                                     break;
                case "Verification Successful"      :  Toast.makeText(ctx,message,Toast.LENGTH_LONG).show();
                    openActivity(designation);
                    break;
                case "problem inserted"      : Toast.makeText(ctx,message,Toast.LENGTH_LONG).show();
                    Intent intent2 = new Intent(ctx,userhome.class);
                    ctx.startActivity(intent2);
                                            break;
                case "got details":   fetchdetails(json);
                    break;
                case "track"      :  Toast.makeText(ctx,message,Toast.LENGTH_LONG).show();
                    Intent intent3 = new Intent(ctx,smhome.class);
    ...
    public void fetchdetails(JSONObject json){
        try {
            ...
            showMessage("request Details",problem_details);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
